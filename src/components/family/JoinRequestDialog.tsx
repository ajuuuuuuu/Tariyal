...
export function JoinRequestDialog({
  ...
  const [relation, setRelation] = useState<"son" | "daughter" | "wife" | "father">("son");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const proposedGender =
      relation === "son" || relation === "father"
        ? "male"
        : relation === "daughter" || relation === "wife"
          ? "female"
          : gender;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Pick your relative in the tree. Admin will review and approve.
          </DialogDescription>
        </DialogHeader>
        <form className="space-y-3" onSubmit={handleSubmit}>
          <div>
            <Label>Search for relative</Label>
            <Input
              value={search}
            ...
            <Select value={parentId} onValueChange={setParentId}>
              <SelectTrigger className="mt-2">
                <SelectValue placeholder="Select your relative" />
              </SelectTrigger>
              <SelectContent>
                {filtered.map((p) => (
          ...
          <div>
            ...
            <Select
              value={relation}
              onValueChange={(v) => setRelation(v as "son" | "daughter" | "wife" | "father")}
            >
